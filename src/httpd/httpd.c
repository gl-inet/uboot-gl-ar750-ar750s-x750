...
static int httpd_findandstore_firstchunk(void){
	...
	if(start){
		...
		end = (char *)strstr((char *)start, "name=\"nand_firmware\"");

		if(end){

			printf("Upgrade type: nand_firmware\n");
			...
			if(end){
				...
				if(end){
					...
					end = (char *)strstr((char *)start, "name=\"nor__firmware\"");

					if(end){

						printf("Upgrade type: nor__firmware\n");
